...
- 节点来源、固定版本、模型哈希和许可说明都在 Toolkit 的[去背景工作流包](https://github.com/Shenrui-Ma/shenrui-comfyui-toolkit/tree/667eafd/workflows/images/rmbg-2-alpha/)（`README.md`、`dependencies.lock.json`）。使用公开`1038lab/ComfyUI-RMBG`仓库的`bd509b4750c81221b938684489c87187b0172208`；不要沿用已经失效的旧组织地址。
- 模型为`1038lab/RMBG-2.0`，revision为`1cd4787601caeb4c8e826dba7ea8e2163b5208df`，放在`ComfyUI/models/RMBG/RMBG-2.0/`。四个文件的URL、SHA256、字节数见该包的 `dependencies.lock.json`。本包不含权重或第三方模型代码。
...
确认模型授权后，按该包 `dependencies.lock.json` 里记录的固定 revision 取这四个文件，逐个核对字节数与 SHA-256：

```
https://huggingface.co/1038lab/RMBG-2.0/resolve/1cd4787601caeb4c8e826dba7ea8e2163b5208df/config.json
https://huggingface.co/1038lab/RMBG-2.0/resolve/1cd4787601caeb4c8e826dba7ea8e2163b5208df/birefnet.py
https://huggingface.co/1038lab/RMBG-2.0/resolve/1cd4787601caeb4c8e826dba7ea8e2163b5208df/BiRefNet_config.py
https://huggingface.co/1038lab/RMBG-2.0/resolve/1cd4787601caeb4c8e826dba7ea8e2163b5208df/model.safetensors
```

已有文件不同就停下核对，不覆盖；网络中断保留未完成文件供核查。该模型约885MB，不属于轻量Skill包。

## 2. 独立去背景图

用 Toolkit 的[去背景包](https://github.com/Shenrui-Ma/shenrui-comfyui-toolkit/tree/667eafd/workflows/images/rmbg-2-alpha/api.template.json)里的参数化API图（`api.template.json`）；本模板不再自带这张图。可用ComfyUI原生节点手动建立等价图：
...
Agent通过API执行时：将原参考图保存至任务独立的ComfyUI input子目录（或`/upload/image`且`overwrite=false`）；根据上传回执把图的 `{{input_image}}` 绑定为真实input相对路径，把 `{{alpha_prefix}}` 与 `{{mask_prefix}}` 绑定为新的任务相对前缀。拒绝绝对路径和`..`。核对当前`/object_info`的参数及三个输出槽位，确认JSON不再含占位符。
