...
当前已提供独立运行入口、完整公开依赖锁与工作流。安装步骤见[install.md](install.md)，当前依赖以[dependencies.lock.json](dependencies.lock.json)为准；[node-sources.md](node-sources.md)保留历史来源，不能当成当前安装清单。封装验收范围见[validation.md](validation.md)。
