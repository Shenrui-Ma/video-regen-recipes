...
受够了在线平台的收费、版权、排队？
本仓库搜集各类二创模板，旨在降低本地视频门槛。无需关心原理，通过可复用的模板与 Skills，教会你的 Agent 部署本地视频工作流，涵盖参考图、语音克隆、后期剪辑。
...
下载完整仓库，在 Codex、Hermes Agent、OpenClaw 等 Agent 中打开项目，然后告诉它你要复刻的视频：

> 复刻【模板名称】，用【自定义提示词/角色参考图】制作。

- Agent 会先询问你的意见，按需下载模型，再一键生成二创视频，建议使用 DeepSeek 等实惠模型（内置Codex模型改DeepSeek的方法）。
- 默认速度优先，采用量化模型 + 少步优化，如果追求质量可告知 Agent 使用多步推理，二次采样和RTX超分。
- 本仓库的 skill 会使用 GPT-Image, Nano Banana, 豆包等生成图像，会根据你的已有登录情况自行选择，也可自定义。
...
| [参考图 Skills](skills/) · [参考视频获取](skills/reference-video-fetch/SKILL.md) · [图片与声音](docs/assets-and-audio.md) · [生图后端怎么选](docs/image-generation-backends.md) | 准备角色图、参考视频与音频 |
...
**投稿模板**：把[模板制造提示词](docs/authoring.prompt.md)连同你的视频、工程、提示词与工作流一起交给你的模型或 Agent，由它按仓库格式整理成模板初稿。
