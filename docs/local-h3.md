...
官方资料将 H3-Base、托管的 Context-IR 预处理和 Regenerate-2K 区分。可选托管预处理属于外部辅助依赖，需要说明素材流向；官方 Regenerate-2K 涉及云端视频再生成，仅作为能力对照，不属于当前本地／自托管模板的收录路线。所需的后续视频生成也应在本地或自托管环境执行；通用超分后的尺寸不能标为 H3 原生同规格生成。
