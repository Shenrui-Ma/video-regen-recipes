...
生图（参考图、改图、换装）的后端不统一：**按用户实际使用的 Agent 产品与模型 provider 选择**——Codex 或走 Codex/GPT 订阅的开源 Agent 优先用 GPT Image 的 CLI/API；其它 provider 先看它的 API 有没有视觉能力；都不行时用用户已登录的 chat（GPT → Gemini → 豆包）；再不行用本地 ComfyUI 或让用户自备图。完整决策顺序与实测链路见[生图后端怎么选](image-generation-backends.md)。
