...
## 5. 产出参考图之后：先给用户看一眼，再往下走

参考图（首帧、换装图、去背景图）是**视频推理的输入**，推下去之后再回头代价高。所以生成完就**立刻把图给用户看，同时说明下一步**，默认继续、不等确认：

> 我做出参考图拿去视频推理咯～你要是不满意就打断我，然后你给我更好的参考图。

规则：

- **先出图，再说明**：把参考图显示出来（或给出本地路径），并说清下一步做什么（哪个模板、哪个模型、大概多久）。
- **默认继续**，不要为了问一句“要不要开始”停下来；用户随时可以打断。
- 用户打断并给出新图或新要求时：**以用户提供的为准**，用新图重做参考图再推理，不拿旧图凑合。
- 用户没有回应就按计划推进；把参考图、提示词与哈希写进项目记录，便于回溯“当时用的哪一张”。
- 参考图来自网页后端时，同时说明它的来源与限制（例如“豆包换装图、只留本机、不再分发”）。
