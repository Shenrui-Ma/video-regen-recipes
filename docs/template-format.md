# 模板格式

以下是建议结构，不要求固定执行器。历史成片、文档收录、执行包可运行与新角色复现分别标注。

```text
templates/<template-id>/
├── README.md     # 效果、素材、步骤、可改范围
├── sources.md    # 作者、参考链接与许可
├── prompts/      # 原提示词与实际提交稿
├── workflows/    # 本地 H3 工作流或固定版本引用
├── assets/       # 可公开的小型素材
└── evidence/     # 配置、结果与失败修复
```

## 最少写清五件事

1. **效果**：保留什么拍法，用户可以改什么。
2. **素材**：图片和声音的要求、用途、文件绑定；工具可本地或在线。
3. **步骤**：从素材准备到 H3 生成、选片与剪辑，怎样局部返修。
4. **配置**：硬件、模型权重、节点版本、工作流和实际参数，参考[H3 说明](local-h3.md)。
5. **实测**：日期、输入、输出、实际时长、耗时、返工与已知限制。

镜头、生成任务和剪辑片段分别编号并记录对应关系，便于只重做受影响的部分。
标明“整理中”“已验证配置”或“待修复”；“已验证”只指实际跑过的环境与输入。
ComfyUI 默认按[本地方式](comfyui-local.md)使用，[远程方式](comfyui-remote.md)单独处理连接与文件传输；模板不写死服务地址或机器路径。
共用内容引用仓库中的 [H3 工作流](../workflows/comfyui/h3/)与[公共方法](../shared/)，不必复制。一个充分验证的配置即可提交。

## Agent 部署与硬件分段

生成模板应让测试者的 Agent 从公开资料完成环境检查、安装、素材绑定、实际生成和导出。给出可下载的工作流，逐个列明非 Core 节点的仓库、固定版本、许可、安装步骤及预检接口；本地自制节点也要提供完整来源。安装成功、离线测试和真实生成分别验收。

生成段数按使用者的 GPU 型号、可用显存、RAM、模型精度、分辨率及实测容量决定。历史作者用了几段只作对照，不写成新任务的固定要求。先按同配置基准预分配，再用短片及真实续接校准；没有可信基准时明确预估，不虚构显存档位或支持保证。加载模型所需的最低容量不足时，增加段数也不能解决。

分段账本分别记录目标时间线、驱动区间、采样长度、上下文、发布裁切、padding 和前驱 latent。段数改变后，提示词时间戳、参考切片、串行队列和后期时间线同步更新；不套用历史固定切点，不擅自降低用户指定质量。

## 加入索引

为 `profile.json` 填写 `index` 信息后，运行 `python3 scripts/catalog.py build` 和 `python3 scripts/catalog.py check`；详细字段见[索引说明](template-index.md)。
